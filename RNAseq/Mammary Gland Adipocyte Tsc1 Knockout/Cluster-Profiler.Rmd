...
# gseaplot2(go.bp, geneSetID = c(1,7,8))

as.data.frame(go.bp) %>%
  mutate(Significant = if_else(qvalue<0.05, 'Significant','NS')) %>%
  mutate(Direction =  if_else(NES > 0, 'Upregulated', 'Downregulated'))  %>%
  group_by(Significant,Direction) %>%
  count %>%
  kable(caption="Summary pf GO-BP pathways")
```
...
```{ wp, fig.cap="Wikipathways enrichment plot"}
# need entrez ids
# 
# 
...
# 
# wp <- gseWP(geneList = names(mouse.gene.list.wp), 
#              verbose=F,
...
```{r transfac, fig.cap="Top enriched pathway."}
library(fgsea)
library(msigdbr)

# Retrieve TRANSFAC gene sets from MSigDB
transfac_genesets <- msigdbr(species = "Mus musculus", category = "C3", subcategory = "TFT:TFT_Legacy")

# Format gene sets for fgsea
transfac_list <- split(transfac_genesets$gene_symbol, transfac_genesets$gs_name)

# Run fgsea with TRANSFAC gene sets
fgsea_results <- fgsea(pathways = transfac_list, 
                       stats = mouse.gene.list,
                       minSize = 15,
                       maxSize = 500)
...
plotEnrichment(transfac_list[[top_pathway]], mouse.gene.list) 
```
