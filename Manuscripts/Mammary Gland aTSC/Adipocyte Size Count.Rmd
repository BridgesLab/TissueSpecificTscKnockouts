...
adipocyte.number <- allmergeddata %>%
  ...
  summarize(adipocyttesum= length(Area), #count
            totalarea= mean( 400/440 *`Total Area`/1000)) %>% #this area is in nm2 (original area was in um then we did / 1000)
...
#getting number by mouse tthen by genotype to graph it
 ...
 averagenumbernormalizedbygenotype <-  adipocyte.numberpermouse %>%
  group_by(Genotype) %>%
  summarize(adipocytenumbergenotpye= mean(adipocyttesum),
            se.adipocytenumbergenotpye=se(adipocyttesum)) #actually averaging the areas up
...
allmergeddata <- allmergeddata %>%
  mutate(Areanm2= 400/440 *Area /1000 )
allmergeddata


ggplot(data=allmergeddata, 
       aes(x=Areanm2, fill=Genotype, na.rm= T ))+
...
library(car)
leveneTest(Areanm2~Genotype, data=allmergeddata) %>%tidy() %>% kable() #p-value <0.001 saying that variability between KO and WT density plots is significant 

#another way to show density area 

ggplot(data=allmergeddata, 
       aes(x=log(Areanm2), fill=Genotype, na.rm= T ))+
  geom_density(alpha=0.8)+
  expand_limits(y=0)+ 
  theme_classic() + 
     scale_fill_grey() +
...
percentilearea <- allmergeddata %>%
  mutate(range=cut(Areanm2, breaks = c(0,0.1,0.2,0.300,0.400,0.500,0.600,0.700,0.800,0.900,1,3.5), right=FALSE))%>% #this shows the range of adipocyte sizes, after looking a the entire range and then breaking itt into deciles and giveing a wrt area 
  group_by(Genotype, range) %>%
  summarize(count=length(Areanm2))%>%
...
ggplot(data=percentilearea, 
  ...
  theme_classic() + 
    ...
    labs(x="Adipocyte Area Range (nm2)",
...
percentileareabymouseonly <- allmergeddata %>%
  mutate(range=cut(Areanm2, breaks = c(0,0.1,0.2,0.300,0.400,0.500,0.600,0.700,0.800,0.900,1,3.5), right=FALSE))%>% #this shows the range of adipocyte sizes, after looking a the entire range and then breaking itt into deciles and giveing a wrt area  old range 0,100,200,300,400,500,600,700,800,900,1000,3500)
  group_by(Genotype,MouseID, range) %>%
  summarize(count=length(Areanm2))%>%
  mutate(totaladipocytes= sum(count)) %>%
  mutate(percentofadipocytes=(count/totaladipocytes *100))
