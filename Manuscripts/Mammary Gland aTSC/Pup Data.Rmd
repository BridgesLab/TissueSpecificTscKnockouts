...
```{r cage-birthsandsurvival} 
  ...
  mutate(Average.Births.Per.Litter = Total/Litter) # INCORRECT!!D!!!!!!!
...
library(ggplot2)
 p <- ggplot(numberofpups.summary,is.na= NA, 
       aes(x=MaternalGenotype, y= Average.Survival,fill= MaternalGenotype, ymax=Average.Survival+SE.Average.Survival, ymin=Average.Survival-SE.Average.Survival))   
...
p + geom_bar(stat='identity', position=position_dodge(), width =0.5) + #match width of Geom_bar with the position dodge of the errorbar 
  geom_errorbar(position=position_dodge(0.5), width=.2) +
...
#t.test(Alive ~ MaternalGenotype, data=numberofpups.per.cage %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in males"))
...
library(ggplot2)
p <- ggplot(bornpups,
aes(x=MaternalGenotype, y= Average.Born.Pups, fill=MaternalGenotype, na.rm =T, ymax= Average.Born.Pups + SE.Average.Born.Pups, ymin=Average.Born.Pups - SE.Average.Born.Pups))
...
library(ggplot2)
 ...
 p + geom_boxplot() +
  labs(title="Total Born Pups Per Cage",
       ...
       x="Maternal ID") +
  theme_classic()
#t.test (Total ~ MaternalGenotype, data=Pupbirth.data %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in males"))
...
library(ggplot2)
 ...
 p + geom_boxplot() +
  ...
  p <- ggplot(numberofpups.per.cage,
       aes(x=MaternalGenotype, y= alive.percent,col= MaternalGenotype))  #alive percent and survival rate are the same thing, one is in % and other is rate
...
t.test(Average.Weight ~ MaternalGenotype, data=Pup.weightspercagePND7 %>% filter(Sex=="Male")) %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in males")  
t.test(Average.Weight ~ MaternalGenotype, data=Pup.weightspercagePND7 %>% filter(Sex=="Female")) %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in females")
```
