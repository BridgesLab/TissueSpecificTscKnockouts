...
p + geom_bar(stat='identity', position=position_dodge(0.5)) +
...
lm(Average.Weight ~ MaternalGenotype,
     data=filter(Pup.weightspercagePND1)) %>% tidy() %>% kable() #KO pups are 0.078/1.286 * 100 = 6.06%  heavier than WT p =0.074 this is at PND1
...
#female WT average PND7 wt= 3.98, female KO average wt= 4.35, effect size= 4.35-3.98/3.98 * 100 = 9.29% 
#male WT weight=4.08, male KO weight=4.27, effect size= 4.27-4.08 /4.08 *100= 4.66% 
...
library(ggplot2)
  ...
  theme_classic()+
    ...
    theme(text=element_text(size=18),
          axis.text.x = element_text(vjust = 0.5, hjust=0.5))+
  theme(legend.position = "none")+
    labs(y="Weight of Pups at PND0.5 (g)",
         x="Dam ID")
  ...
  #labs(x="Maternal Genotype",
       #y="Weight (g)",
       #title="Weight of Pups on PND0.5") +
  expand_limits(y=0) +
  theme_classic() +
  scale_fill_grey() +
    scale_color_grey() +
...
library(ggplot2)
  ...
  scale_fill_grey() +
    ...
    theme(text=element_text(size=18),
          axis.text.x = element_text(vjust = 0.5, hjust=0.5))+
  theme(legend.position = "none")+
    labs(y="Weight of Pups at PND7.5 (g)",
         x="Dam ID")
...
lm(Average.Weight ~ MaternalGenotype + Sex, data=Pup.weightspercagePND7 ) %>% tidy %>% kable(caption="Pup weight at PND7, including adjustment by sex") #p=0.010 for KO genotype

#within group 
t.test(Average.Weight ~ MaternalGenotype, var.equal=T, data=Pup.weightspercagePND7 %>% filter(Sex=="Male")) %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in males")  #p=0.142
t.test(Average.Weight ~ MaternalGenotype, data=Pup.weightspercagePND7 %>% filter(Sex=="Female"),
       var.equal=T) %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on PND7 weights in females") #p=0.044
```
...
library(ggplot2)
  ...
  scale_fill_grey() +
    ...
    theme(text=element_text(size=18),
          axis.text.x = element_text(vjust = 0.5, hjust=0.5))+
  theme(legend.position = "none")+
    labs(y="Weight of Pups at PND14.5 (g)",
         x="Dam ID")
 ...
 # labs(x="Dam ID",
  ...
  scale_fill_grey() +
    ...
    theme(text=element_text(size=18),
         axis.text.x = element_text(vjust = 0.5, hjust=0.5))+
  theme(legend.position = "none")+
    labs(y="Weight of Pups at PND16.5 (g)",
         x="Dam ID")
...
pupbodyweight <-
  ...
  pupaveragewtpermaternalgenotype <- pupbodyweight%>%
    ...
    mutate(AverageBWpermaternalgenotype= mean(AverageBW),
...
t.test(BW ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Male")) %>% tidy %>% kable() 
t.test(BW ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Female")) %>% tidy %>% kable()
t.test(Fat ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Male")) %>% tidy %>% kable()
t.test(Fat ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Female")) %>% tidy %>% kable()
t.test(Lean ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Male")) %>% tidy %>% kable()
t.test(Lean ~ MaternalGenotype, data=PupMRI %>% filter(Sex=="Female")) %>% tidy %>% kable()

#Analysis MRI not sex specific
pupPND16MRIdata <-
  PupMRI %>% 
  group_by(MaternalGenotype) %>%
  mutate(AverageBW= mean(BW)) %>%
  ...
  pupMRInotbysex <- pupPND16MRIdata%>%
    group_by(MaternalGenotype) %>%
    mutate(AverageBWpermaternalgenotype= mean(AverageBW),
              AverageLeanpermaternalgenotype= mean(AverageLean),
              AverageFatpermaternalgenotype= mean(AverageFat))
...
t.test(BW ~ MaternalGenotype, data=PupMRI) %>% tidy %>% kable() 
t.test(Fat ~ MaternalGenotype, data=PupMRI) %>% tidy %>% kable()
t.test(Lean ~ MaternalGenotype, data=PupMRI) %>% tidy %>% kable()
```
