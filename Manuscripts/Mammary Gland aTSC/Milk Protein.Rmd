...
proteincomp2 <- Milkproteinbatch2 %>%  #this is using the raw area values from the LiCOR machine
  group_by(Genotype, Protein) %>%
  filter(Protein!= "Band1") %>%
  filter(Protein!= "Band2") %>%
  filter(Protein!= "Band6") %>%
  filter(Protein!= "Band8") %>%
summarize(Average.Protein = mean(Value, na.rm=T),
        SE.Average.Protein = se(Value))
kable(proteincomp2)
...
p + geom_bar(stat='identity', position=position_dodge(0.5), width =0.5) +
  ...
  labs(title="Protein in Milk of KO and WT Dams") + #from BATCH 2 only
  expand_limits(y=0) +
  theme_classic() +
     theme(text = element_text(size=13)) 
...
t.test(Value~Genotype, data=Milkproteinbatch2) %>% tidy %>% kable(caption="Welch's t-test for effects of maternal genotype on total milk protein")
```
