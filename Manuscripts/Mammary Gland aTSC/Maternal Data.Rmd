...
maternal.weights <-
  ...
  labs(x = "Days From Delivery (0)", y= "Body Weight (g)", title="Body Weights of Dams during Gestation and Lactation") 
...
## The effect size is -0.102*16= -1.632 grams weight loss over 16 days postnatal for KO dams compared to WT. This is not a significant effect with p-value= 0.061
summary(pnbw.lme)
```
...
## The effect size is 0.125*16= 2 grams additional weight gain over 16 days postnatal for KO dams compared to WT. This is a significant effect with p-value <0.001
```
...
maternal.weights <-
  ...
  theme_classic() +
       theme(text = element_text(size=13)) +
  labs(x = "Days From Delivery (0)", y= "Lean Mass (g)", title="Lean Mass Change from Delivery Day and during Lactation")
...
## The effect size is -0.167*16= -2.672 grams additional lean mass loss over 16 days postnatal for KO dams compared to WT. This is not a significant effect with p-value= 0.404
```
...
maternal.weights <-
  ...
  ggplot(data=maternal.weights, 
       aes(y= FoodIntake,
           x=Week,
           col=MaternalGenotype, na.rm =T, xmin=1, xmax=7, ymin=20, ymax=45)) +
 ...
 #facet_grid(~Treatment) +
  ...
  labs(x="Week", y="Average Food Intake (g)", title="Average Dam Weekly Food Intake before amd after Delivery at around Week 4")
  ...
  ggplot(data=foodintakefromdelivery,
         aes(y=FoodIntake,
             x=DayfromDelivery,
             col=MaternalGenotype, na.rm=T,
             fill=MaternalGenotype, na.rm =T, ymin=0, ymax=80, xmin=0, xmax= 21))+
       scale_x_continuous(breaks=seq(0,21,1)) +
  ...
  theme_classic() +
       theme(text = element_text(size=13)) +
  labs(x = "Days From Delivery (0)", y= "Lean Mass (g)", title="Food Intake from Delivery Day and during Lactation")
...
## The effect size is -6.34*16= -101.44 grams less food consumption over 16 days postnatal for KO dams compared to WT. This is a significant effect with p-value = 0.0113
```
...
write.csv(maternal.feedingefficiency, file = "MyData.csv");
 ...
 p <- ggplot(maternal.feedingefficiency, is.na= NA,
       aes(y= feedingeff,
           x=Week,
           col=MaternalGenotype, na.rm =T, xmin=0, xmax=7, ymin=-200, ymax=100)) +
      scale_x_continuous(breaks=seq(0,7,0.5))+
 ...
 #facet_grid(~Treatment) +
  ...
  #facet_grid(~Cohort)+
  labs(x="Week", y="Feeding Efficiency",title="Feeding Efficiency of Dams per Treatment")   
p
