...
```{r time-course-aggregating}
daily.animal.time.course <-
  animal.time.course %>%
  mutate(Diet = if_else(Interval < 172, "NCD", "HFD")) %>%
  group_by(Diet, Time, Treatment) %>%
  summarize_at(.vars=vars(VO2:Total.Activity), .funs=list(mean=mean,se=se))
...
```{r heat-time-course, fig.cap="The respiratory exchange ratio over time."}
with(subset(animal.time.course, Treatment==levels(merged_data$Treatment)[1]), plot(Time.Elapsed,HEAT, type="l", xlab="Time (h)", ylab="Heat Production (W)", las=1))
with(subset(animal.time.course, Treatment==levels(merged_data$Treatment)[2]), lines(Time.Elapsed,HEAT, col=palette()[2]))
with(subset(animal.time.course, Treatment==levels(merged_data$Treatment)[3]), lines(Time.Elapsed,HEAT, col=palette()[3]))
legend("topleft", levels(merged_data$Treatment), bty="n", lty=1, col=palette()[1:2])

library(ggplot2)
daily.animal.time.course %>%
  filter(Diet=='HFD') %>%
  ggplot(aes(y=HEAT_mean,
             ymin=HEAT_mean-HEAT_se,
             ymax=HEAT_mean+HEAT_se,
             x=Time,col=Treatment)) +
  geom_line() +
  geom_errorbar() +
  labs(y="Heat Production (W)",
       x="Time (24h clock",
       title="Energy Expenditure After 1-3 Days HFD") +
