...
```{r data-input}
google_drive_file <- 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSth-DS9iN3peHkt9-C8vz2pNWKSw2Y_rSCNCLq-7DIbzMZ_8rjLITffQjLT3jw7lEUFfacXQeakkEB/pub?gid=158703896&single=true&output=csv'
...
data <- mutate(exp.data, 
  ...
  mutate(Date = mdy(Date),
    ...
    mutate(Injection = ifelse(Mouse %in% not.ko, "GFP", as.character(Injection))) #modified genotypes based on western blotting
...
These data can be found in **`r getwd()`** in a file named **`r ifelse(filename %in% dir(), filename, "no file found")`**, which was downloaded from a google sheet found at `r google_drive_file`.  This script was most recently updated on **`r date()`**.
