...
exp.data <- read_excel(data.filename, sheet='Sheet2') %>%
  ...
  mutate(Injection = ifelse(ID %in% not.ko, "GFP", as.character(Injection)))  #modified genotypes based on western blotting
...
summary.data <-
  exp.data %>%
  group_by(Sex,Diet,Injection) %>%
  summarize_if(is.numeric, .funs=list(mean = function(x) mean(x,na.rm=T),
                                      se=se))
```

These data can be found in **/Users/katherinekistler/Documents/GitHub/TissueSpecificTscKnockouts/Mouse Data/Liver AMPK Ketogenic Diet/All Figures/ITT** in a file named **ITT 19-06-19.xlsx** and **mapping.csv**.  This script was most recently updated on **Mon May 18 12:32:52 2020**.
...
## (Intercept)         136.550      7.408  92.132   18.43  < 2e-16 ***
...
## InjectionGFP         -0.515      6.437  63.045   -0.08     0.94    
## ---
...
## as.fct(T)15 -0.315                                                   
## as.fct(T)30 -0.315  0.500                                            
## as.fct(T)45 -0.315  0.500   0.500                                    
## as.fct(T)60 -0.314  0.498   0.498  0.498                             
## as.fct(T)75 -0.314  0.498   0.498  0.498  0.497                      
## as.fct(T)90 -0.314  0.498   0.498  0.498  0.497  0.497               
## as.fc(T)105 -0.314  0.498   0.498  0.498  0.497  0.497  0.497        
## as.fc(T)120 -0.314  0.498   0.498  0.498  0.497  0.497  0.497  0.497 
## DietKeto    -0.465  0.000   0.000  0.000 -0.002 -0.002 -0.002 -0.002 
## SexM        -0.468  0.000   0.000  0.000  0.002  0.002  0.002  0.002 
## InjectinGFP -0.466  0.000   0.000  0.000  0.002  0.002  0.002  0.002 
##             a.(T)12 DietKt SexM  
## as.fct(T)15                      
## as.fct(T)30                      
## as.fct(T)45                      
...
-------------  ---------  ----------  ----------  --------
(Intercept)        149.1        6.37       23.39     0.000
DietKeto            28.3        6.11        4.63     0.000
SexM                11.0        6.12        1.80     0.077
InjectionGFP       -10.9        6.10       -1.78     0.080
...
term            estimate   std.error   statistic   p.value
-------------  ---------  ----------  ----------  --------
(Intercept)      944.378        60.7      15.566     0.000
DietKeto         416.244        58.6       7.103     0.000
SexM              67.238        58.7       1.146     0.256
InjectionGFP       0.468        58.5       0.008     0.994
