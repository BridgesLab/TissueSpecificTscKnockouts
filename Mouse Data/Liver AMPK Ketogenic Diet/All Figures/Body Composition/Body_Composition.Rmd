...
exp.data <- read_csv(raw_data_file, col_types = 
  ...
  mutate(Injection = ifelse(MouseID %in% not.ko, "AAV-GFP", as.character(Injection))) %>% #modified genotypes based on western blotting
...
This script pulled in a total of `r dim(exp.data)[1]` observations from cohort `r cohort_id`.  This includes the following number of animals in each treatment group.
