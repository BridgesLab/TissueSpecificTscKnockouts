...
library(lme4)
library(lmerTest)
lmer(Glucose ~ as.factor(Time) + Genotype *Sex + (1|MouseID), data=itt.data.long %>% filter(!is.na(Time))) %>% anova %>% kable(caption="Mixed linear models for effects on ITT")
```
