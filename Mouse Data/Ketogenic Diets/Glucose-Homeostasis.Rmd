...
fg.summary %>%
  ...
  geom_errorbar(width=0.5, aes(ymin=Avg-SE,ymax=Avg+SE,)) +
  facet_grid(.~Sex) +
  labs(title="Fasting Glucose (6h Fast)",
       y="Blood Glucose (mg/dL)",
       x="Genotype")
...
fg.summary %>%
  ggplot(aes(y=Avg,
             ...
             fill=Genotype)) +
  geom_bar(stat='identity') +
  geom_errorbar(width=0.5, aes(ymin=Avg-SE,
             ymax=Avg+SE,)) +
  facet_grid(.~Sex) +
  labs(title="",
...
glucose.data %>%
  ...
  geom_bar(stat='identity') +
  facet_grid(~Sex) +
  geom_errorbar(width=0.5, aes(ymin=AUC.mean-AUC.se,ymax=AUC.mean+AUC.se, )) +
  labs(title="",
