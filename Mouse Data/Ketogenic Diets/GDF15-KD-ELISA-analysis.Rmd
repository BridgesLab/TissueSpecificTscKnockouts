...
summary.data <- 
  exp.data %>%
  group_by(Diet,Sex,Cohort,Treatment) %>%
...
summary.data.no.cht <- 
  exp.data %>%
  group_by(Diet,Sex,Treatment) %>%
  summarize(GDF15 = mean(Conc., na.rm=T),
            Error = se(Conc.),
            N = length(Conc.))
...
t.test(Concentration ~ Diet, filter(kd.data, Sex=="F"), var.equal=T) #Welch vs Student is var.equal=T/F
...
t.test(Concentration ~ Diet, filter(kd.data, Sex=="M"), var.equal=T) #Welch vs Student is var.equal=T/F


summary.data.no.cht %>% filter(Diet %in% c('CD','KD')) %>% select(-Treatment) %>% kable(caption="Summary statistics for effects of a ketogenic diet")
