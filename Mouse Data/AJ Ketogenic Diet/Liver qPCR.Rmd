...
#removed bad wells from third plate
bad.wells.3 <- c('O6','O8','P6','P8','M6','K6','O5','N3','A10','N4','P4','G1')
#plate layout is wrong, recheck this.
...
combined.data.clean <- 
  ...
  filter(!(`Sample`==4116)) #removed no amplification
...
qpcr.results <-
  qpcr.data %>%
  group_by(`Sample`,`Target`, `Diet.x`, Sex) %>%
...
sample.results <-
  ...
  group_by(`Target`) %>%
  mutate(Rel.norm = Quant.norm/mean(Quant.norm[`Diet.x`==control.group&Sex==control.sex], na.rm=T))

summary.results <-
  sample.results %>%
  group_by(`Target`, `Diet.x`, Sex) %>%
...
p + geom_boxplot(aes(color=`Diet.x`)) + 
  geom_point(aes(color=`Diet.x`), position=position_dodge(width=0.75)) +
...
kable(filter(sample.results, `Target`==gene) %>% arrange(`Diet.x`, Sex, Rel.norm) %>% select(`Diet.x`, Sex, `Sample`, Ct.mean, Ct.range, Rel.norm), caption="BDH1 Expression on a per sample basis")
...
lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='BDH1')) %>% summary

lm(Rel.norm ~ Diet.x + Sex, data=sample.results  %>%
    filter(Target=='BDH1')) %>% summary

lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='BDH1' & Sex=='F')) %>% summary
```

```{r bdh1-barplot}
p <- ggplot(
  filter(summary.results, `Target`==gene),
  aes(y=Expression, group=`Diet.x`,x=Sex))
p + geom_bar(position=position_dodge(), stat="identity",aes(fill=Diet.x)) +
  geom_errorbar(aes(ymin=Expression-Expression.se, ymax=Expression + Expression.se,),
...
kable(filter(sample.results, `Target`==gene) %>% arrange(`Diet.x`, Sex,Rel.norm) %>% select(`Diet.x`, Sex, `Sample`, Ct.mean, Ct.range, Rel.norm), caption="Hmgcs2 Expression on a per sample basis")
...
lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='Slc16a6V2')) %>% summary

lm(Rel.norm ~ Diet.x + Sex, data=sample.results  %>%
    filter(Target=='Slc16a6V2')) %>% summary

lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='Slc16a6V2'& Sex=='M')) %>% summary
```
...
kable(filter(sample.results, `Target`==gene) %>% arrange(`Diet.x`, Sex, Rel.norm) %>% select(`Diet.x`, Sex, `Sample`, Ct.mean, Ct.range, Rel.norm), caption="HMGCL Expression on a per sample basis")
...
lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='HMGCL_2')) %>% summary

lm(Rel.norm ~ Diet.x + Sex, data=sample.results  %>%
    filter(Target=='HMGCL_2')) %>% summary

lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='HMGCL_2' & Sex=='F')) %>% summary
```
...
lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='HMGCS2_2')) %>% summary

lm(Rel.norm ~ Diet.x + Sex, data=sample.results  %>%
    filter(Target=='HMGCS2_2')) %>% summary

lm(Rel.norm ~ Diet.x, data=sample.results  %>%
    filter(Target=='HMGCS2_2' & Sex=='F')) %>% summary
```
